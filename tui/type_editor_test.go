...
func TestTypeEditor_TotalItems(t *testing.T) {
	te := newTypeEditor(testSchema(), "book", false, nil)
	// 6 meta fields + 4 properties + 1 add property + 0 templates + 1 add template + 0 views + 1 add view = 13
	if te.totalItems() != 13 {
		t.Errorf("totalItems = %d, want 13", te.totalItems())
	}
}
...
func TestTypeEditor_DisplayItems(t *testing.T) {
	...
	// Should have 13 items: 6 meta + 2 pinned + 2 unpinned + 1 add property + 1 add template + 1 add view
	if len(items) != 13 {
		t.Fatalf("len(displayItems) = %d, want 13", len(items))
	}
